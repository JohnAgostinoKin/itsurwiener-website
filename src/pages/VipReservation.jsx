...
export default function VipReservation() {
  ...
  useEffect(() => {
    Promise.all([
      client.fetch(`*[_type == "reservationPackages"][0]{ vipPackages, availableTimes, vipNote }`),
      client.fetch(`*[_type == "vipReservation"][0]`),
    ]).then(([pkgData, settingsData]) => {
      if (pkgData?.vipPackages?.length) setPackages(pkgData.vipPackages.filter(p => p.available !== false))
        ...
        <motion.div initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} className="mb-10">
          ...
          <div className="border border-purple-bright/10 p-6 md:p-8" style={{ background: 'rgba(157,78,221,0.02)' }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-5">
              ...
              <div>
                <label className="font-ui text-[9px] font-bold tracking-[.22em] uppercase text-purple-bright/60 block mb-2">Time *</label>
            ...
            <div className="mb-5">
              <label className="font-ui text-[9px] font-bold tracking-[.22em] uppercase text-purple-bright/60 block mb-2">Guest Count *</label>
              <select value={guests} onChange={e=>setGuests(e.target.value)}
                className="w-full bg-[#08060F] border border-white/10 text-cream font-ui text-[14px] px-4 py-3.5 outline-none focus:border-purple-bright transition-colors">
                <option value="">Select guests...</option>
                {Array.from({length: selected?.maxGuests || 40},(_,i)=>i+1).map(n=><option key={n}>{n} {n===1?'Guest':'Guests'}</option>)}
